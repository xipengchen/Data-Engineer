...
def download_Amazon_traffic_data_upload_to_s3(ds, **kwargs):
    ...
    if len(dates_to_fetch) > 0:
        ...
        for input_date in sorted(dates_to_fetch):
            ...
            # To check the date within the downloaded file is same as input_date
            if str(input_date) == datetime.strptime(pd.read_csv('/home/ec2-user/airflow_dags_results/Amazon_US_traffic/Traffic Diagnostic_Detail View_US.csv', skiprows=0,encoding='utf-8').columns[6].split('=')[1].split('-')[0].replace('[','').encode().strip(),'%m/%d/%y').strftime('%Y-%m-%d'):
            ...
            # if not the same date, delete the file                          
            else:
                os.remove('/home/ec2-user/airflow_dags_results/Amazon_US_traffic/Traffic Diagnostic_Detail View_US.csv')                                
...
def upload_aggregate_s3_traffic_data_to_reshift(ds, **kwargs):
    ...
    if len(dates_to_upload) > 0:
        ...
        for input_date in sorted(dates_to_upload):
            ...
            delete_file = '/home/ec2-user/airflow_dags_results/Amazon_US_traffic/Download_monthly/Amz_traffic_{}.csv'.format(input_date.strftime("%Y%m%d"))
